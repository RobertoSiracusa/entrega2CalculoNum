...
class ArchiveUtil:
    def __init__(self, router=""):
        if not router or not router.strip():
            print("La ruta asignada no es válida.")
        self._router = None
    ...
    def setCreateArchiveTxt(self, content, fileName, append_newline=False):
        ...
        if not content or not content.strip(): 
            from Helpers.utils import logWriter
            logWriter("El contenido es requerido.")
        if not fileName:
            logWriter("El nombre del archivo es requerido.")
    ...
    def logError(self, error_type, error_message):
        try:
            ...
            self.setCreateArchiveLog(log_entry, " ", append_newline=True, booleano=False)
