...
from Helpers.utils import format2DArray, logWriter, txtWriter
...
def storeMatrix(dataArray,outputFileName):
    try:
        output=format2DArray(dataArray)
        txtWriter(outputFileName, output, True)
    except Exception as e:
        logWriter(f"Error en Gauss-Jordan: {e}",True)